...
@Validated
@Slf4j
public class AuthenticatedNorthCountriesController {

    private final NorthCountriesService northCountriesService;

    @Autowired
    public AuthenticatedNorthCountriesController(NorthCountriesService northCountriesService) {
        this.northCountriesService = northCountriesService;
    }

    /**
     * Rest API GET request Handler for authenticated northern hemisphere ip requests.
    ...
    public NorthCountriesResponseDto getAuthenticatedNorthCountries(@RequestParam(value = "ip")
        ...
        log.debug("retrieving authenticated northern hemisphere countries by ip...");
        log.debug("request parameters: " + ipAddresses);
        List<IpAddress> ipAddressList = convertToIpList(ipAddresses);
        return convertToNorthCountriesResponseDto(northCountriesService.getAuthenticatedNorthCountries(ipAddressList));
    }
}
